...
def create_fake_params(config: transformer_lib.TransformerConfig):
  ...
  # repeated = jnp.stack([value] * config.num_layers, axis=0)
  ...
  shape = (config.num_heads, config.head_dim, config.embed_dim)
  value = jax.random.normal(key, shape)
  params['layers']['attn']['attn_vec_einsum']['w'] = jnp.stack([value] * config.num_layers, axis=0)
    # jnp.ones(
    #     (config.num_layers, config.num_heads, config.head_dim, config.embed_dim)
    # )
  if config.num_heads == config.num_kv_heads:
    shape = (3, config.num_heads, config.embed_dim, config.head_dim)
    value = jax.random.normal(key, shape)
    params['layers']['attn']['qkv_einsum']['w'] = jnp.stack([value] * config.num_layers, axis=0)
      # jnp.ones(
      #     (config.num_layers, 3, config.num_heads, config.embed_dim, config.head_dim)
      # )
  else:
    shape = (config.num_heads, config.embed_dim, config.head_dim)
    value = jax.random.normal(key, shape)
    params['layers']['attn']['q_einsum']['w'] = jnp.stack([value] * config.num_layers, axis=0)
      # jnp.ones(
      #     (config.num_layers, config.num_heads, config.embed_dim, config.head_dim)
      # )
    params['layers']['attn']['kv_einsum']['w'] = jnp.stack([value] * config.num_layers, axis=0)
      # jnp.ones(
      #     (config.num_layers, config.num_kv_heads, config.embed_dim, config.head_dim)
      # )

  # 4. feedforward block params
  shape = (2, config.embed_dim, config.hidden_dim)
  value = jax.random.normal(key, shape)
  params['layers']['mlp']['gating_einsum'] = jnp.stack([value] * config.num_layers, axis=0)
    # jnp.ones(
    #     (config.num_layers, 2, config.embed_dim, config.hidden_dim)
    # )
  shape = (config.hidden_dim, config.embed_dim)
  value = jax.random.normal(key, shape)
  params['layers']['mlp']['linear'] =  jnp.stack([value] * config.num_layers, axis=0)
    # jnp.ones(
    #     (config.num_layers, config.hidden_dim, config.embed_dim)
    # )

  # 5. layer norm params
  shape = (config.embed_dim,)
  value = jax.random.normal(key, shape)
  params['layers']['pre_attention_norm']['scale'] = jnp.stack([value] * config.num_layers, axis=0)
    # jnp.ones((
    #     config.num_layers, config.embed_dim,
    # ))
  params['layers']['pre_ffw_norm']['scale'] = jnp.stack([value] * config.num_layers, axis=0)
    # jnp.ones((
    #     config.num_layers, config.embed_dim,
    # ))

  if config.use_post_attn_norm:
    params['layers']['post_attention_norm']['scale'] = jnp.stack([value] * config.num_layers, axis=0)
      # jnp.ones((
      #     config.num_layers, config.embed_dim,
      # ))
  if config.use_post_ffw_norm:
    params['layers']['post_ffw_norm']['scale'] = jnp.stack([value] * config.num_layers, axis=0)
      # jnp.ones((
      #     config.num_layers, config.embed_dim,
      # ))
  return res
...
class TransformerTest(parameterized.TestCase):
  ...
  #         expected_cache_shape=(3, 5, 3, 2, 2), # num_layers, batch_size, cache_size, num_heads, head_dim
  #     ),
  # )
